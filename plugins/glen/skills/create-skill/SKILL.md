...
1. **Pick the destination FIRST — local file or Glen skill.** A skill is either a **local `SKILL.md`** on this machine (a Claude Code skill, visible only to you) or a **Glen skill** saved to your Glen library (recorded; private to you by default, sharable with teammates or the whole org).
   ...
   - **Otherwise, ask the user which they want** before authoring anything: a local `SKILL.md` (a Claude skill on this machine) or a Glen skill (saved to your Glen library). Don't start writing until they answer.
...
5. **Finish based on the destination chosen in step 1:**
   ...
   **Local skill** (and always the incognito path) — nothing leaves the machine:
   - Save it to `.claude/skills/<name>/SKILL.md` in this project — or `~/.claude/skills/<name>/SKILL.md` for a personal skill that follows you across every project.
